...
local root = "/app/software/WebKitGTK+/2.27.4-GCC-8.3.0"
...
if not ( isloaded("gperf/3.1-GCCcore-8.3.0") ) then
    load("gperf/3.1-GCCcore-8.3.0")
end
...
if not ( isloaded("LibSoup/2.70.0-GCCcore-8.3.0") ) then
    load("LibSoup/2.70.0-GCCcore-8.3.0")
end
...
if not ( isloaded("libgcrypt/1.8.5-GCCcore-8.3.0") ) then
    load("libgcrypt/1.8.5-GCCcore-8.3.0")
end

if not ( isloaded("libwebp/1.1.0-GCCcore-8.3.0") ) then
    load("libwebp/1.1.0-GCCcore-8.3.0")
end
...
if not ( isloaded("libtasn1/4.16.0-GCCcore-8.3.0") ) then
    load("libtasn1/4.16.0-GCCcore-8.3.0")
end
...
if not ( isloaded("Ruby/2.7.1-GCCcore-8.3.0") ) then
    load("Ruby/2.7.1-GCCcore-8.3.0")
end
...
setenv("EBVERSIONWEBKITGTKPLUS", "2.27.4")
setenv("EBDEVELWEBKITGTKPLUS", pathJoin(root, "logs/WebKitGTK+-2.27.4-GCC-8.3.0-easybuild-devel"))
