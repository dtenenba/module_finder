...
local root = "/app/software/ZeroMQ/4.3.2-GCCcore-9.3.0"
...
if not ( isloaded("OpenPGM/5.2.122-GCCcore-9.3.0") ) then
    load("OpenPGM/5.2.122-GCCcore-9.3.0")
end

if not ( isloaded("libsodium/1.0.18-GCCcore-9.3.0") ) then
    load("libsodium/1.0.18-GCCcore-9.3.0")
end

if not ( isloaded("util-linux/2.35-GCCcore-9.3.0") ) then
    load("util-linux/2.35-GCCcore-9.3.0")
end
...
setenv("EBVERSIONZEROMQ", "4.3.2")
setenv("EBDEVELZEROMQ", pathJoin(root, "logs/ZeroMQ-4.3.2-GCCcore-9.3.0-easybuild-devel"))
